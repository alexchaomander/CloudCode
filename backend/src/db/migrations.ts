...
export function runMigrations(): void {
  ...
  try {
    const tableInfo = db.prepare("PRAGMA table_info(sessions)").all() as unknown as { name: string }[];
  ...
  const existingProfiles = db.prepare('SELECT COUNT(*) as count FROM agent_profiles').get() as unknown as { count: number } | undefined;
