...
const profileRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/api/v1/profiles', { preHandler: requireAuth }, async (request, reply) => {
    const rows = db.prepare('SELECT * FROM agent_profiles ORDER BY name ASC').all() as AgentProfile[];
  ...
  fastify.post('/api/v1/profiles', { preHandler: requireAdmin }, async (request, reply) => {
    ...
    const row = db.prepare('SELECT * FROM agent_profiles WHERE id = ?').get(id) as AgentProfile;
  ...
  fastify.get('/api/v1/profiles/:id', { preHandler: requireAuth }, async (request, reply) => {
    ...
    const row = db.prepare('SELECT * FROM agent_profiles WHERE id = ?').get(id) as AgentProfile | undefined;
  ...
  fastify.put('/api/v1/profiles/:id', { preHandler: requireAdmin }, async (request, reply) => {
    const { id } = request.params as { id: string };
    const existing = db.prepare('SELECT * FROM agent_profiles WHERE id = ?').get(id) as AgentProfile | undefined;
    ...
    const updated = db.prepare('SELECT * FROM agent_profiles WHERE id = ?').get(id) as AgentProfile;
