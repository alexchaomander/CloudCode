...
function getAllSettings(): Record<string, string> {
  const rows = db.prepare('SELECT key, value FROM settings').all() as unknown as Array<{ key: string; value: string }>;
...
const DEPRECATED_KEYS = new Set(['terminal_poll_interval_ms']);
...
const settingsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/api/v1/settings', { preHandler: requireAuth }, async (request, reply) => {
    ...
    for (const [key, value] of Object.entries(all)) {
      if (!SENSITIVE_KEYS.has(key) && !DEPRECATED_KEYS.has(key)) {
        safe[key] = value;
      }
    }
